...
var __mqttBundle = (() => {
  ...
  function mapState(internal) {
    if (internal === null || internal === void 0) return void 0;
    return STATE_MAP[internal] ?? internal;
  }
  function mapSubstate(internal) {
    if (internal === null || internal === void 0) return void 0;
    return SUBSTATE_MAP[internal] ?? internal;
  }
  function deriveWakeState(publishedState) {
    return publishedState !== "Sleep";
  }
  function deriveSteamFields(publishedState, steamDisabled, ecoSteamOn) {
  ...
  var WATER_TANK_FULL_ML = 2058;
  function waterTankLevelToMilliliters(mm) {
...
`) && (e = e.replace(Kx, "%0A")), e.includes("\r") && (e = e.replace(Qx, "%0D")), e.includes("	") && (e = e.replace(Yx, "%09")), n.pathname = e, n;
  ...
  var TinyEmitter = class {
    ...
    on(event, cb2) {
      ...
      this._listeners.get(event).add(cb2);
      return this;
    }
    once(event, cb2) {
      const wrapper = (...args) => {
    ...
    removeListener(event, cb2) {
      this._listeners.get(event)?.delete(cb2);
      return this;
    }
    ...
    emit(event, ...args) {
      ...
      if (!set) return false;
      for (const cb2 of Array.from(set)) cb2(...args);
      return true;
    }
  };
  var HostTransportStream = class extends TinyEmitter {
    ...
    _flush() {
      ...
      const sendPromise = this._openPromise ? this._openPromise.then(() => this._hostTransport.send(this._handle, payload)) : this._hostTransport.send(this._handle, payload);
      sendPromise.then(() => {
  ...
  var MAX_RECONNECT_DELAY_MS = 64e3;
  function createMqttBridge({ host, config, onCommand, log }) {
    ...
    function start() {
      ...
      client = new mqtt_esm_default.MqttClient(openStream, {
        clientId: config.clientId,
        username: config.username || void 0,
        password: config.password || void 0,
        clean: true,
        reconnectPeriod: 0,
        keepalive: Math.ceil((config.publishIntervalMs + 3e3) / 1e3),
        protocolVersion,
        will: {
          topic: stateTopic,
  ...
  function createLoopbackJsonStream({ host, path, onJson, onStatus, log }) {
    ...
    function scheduleReconnect() {
      ...
      backoffMs = Math.min(backoffMs * 2, 64e3);
    }
...
var createPlugin = __mqttBundle.createPlugin;
