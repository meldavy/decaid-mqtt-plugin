...
test("dispatcher maps commands to loopback REST calls", async () => {
  const stateProviderCalls = [];
  const { fetchImpl, calls } = mockFetch([
    {
      match: /^http:\/\/localhost:8080\/api\/v1\/machine\/state\/heating$/,
      method: "PUT",
  ...
  assert.equal(calls[0].url, "http://localhost:8080/api/v1/machine/state/heating");
});
...
test("steam_on wakes the machine first when asleep", async () => {
  const { fetchImpl, calls } = mockFetch([
    {
      match: /\/state\/(heating|steam)$/,
      method: "PUT",
      respond: () => ({ ok: true, status: 200 }),
    },
  ]);
  const dispatcher = new CommandDispatcher({
    fetchImpl,
    currentStateProvider: () => "Sleep",
  });
  await dispatcher.dispatch({ kind: "steam_on", argument: null });
  assert.deepEqual(
    calls.map((c) => c.url),
    [
      "http://localhost:8080/api/v1/machine/state/heating",
