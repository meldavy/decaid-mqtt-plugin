...
test("wake command puts the machine into heating", async () => {
  const { broker, sim } = env;
  await waitFor(() => broker.subscriptions.length > 0);
  await sendCommand(broker, "wake");
  await waitFor(() => sim.requests.some((r) => r.method === "PUT" && r.path === "/api/v1/machine/state/heating"));
});
...
test("steam_on wakes a sleeping machine before steaming", async () => {
  const { broker, sim, plugin } = env;
  await waitFor(() => broker.publishes.some((p) => p.topic.endsWith("/state")));
  ...
  await waitFor(() => {
    const puts = sim.requests.filter((r) => r.method === "PUT");
    return puts.some((r) => r.path === "/api/v1/machine/state/heating")
      && puts.some((r) => r.path === "/api/v1/machine/state/steam");
  });
  const puts = sim.requests.filter((r) => r.method === "PUT");
  assert.equal(puts[0].path, "/api/v1/machine/state/heating");
  assert.equal(puts[1].path, "/api/v1/machine/state/steam");
});

test("steam_off puts the machine to idle", async () => {
  const { broker, sim } = env;
  await waitFor(() => broker.subscriptions.length > 0);
